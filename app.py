...
def oil_effect(decode_array_to_img):
    bgr_img=cv2.cvtColor(decode_array_to_img,cv2.COLOR_RGBA2BGR)
    oil_effect_img = cv2.xphoto.oilPainting(bgr_img, 7, 1)
    rgba_img=cv2.cvtColor(oil_effect_img,cv2.COLOR_BGR2RGBA)
...
def HDR(decode_array_to_img):
    ...
    status, output_img = cv2.imencode('.JPG', hdr_effect)
    return output_img.tobytes()
# Code ends
